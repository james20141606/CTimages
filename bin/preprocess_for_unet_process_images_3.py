...
filename_ = np.loadtxt('preprocess/forunet/images/nega_3/a.txt',dtype='S')
#1-39
count = int(args.number)
filename = filename_[(count-1)*25:count*25]
for i in tqdm(range(filename.shape[0])):
    img_file =filename[i]
    imgs_to_process = np.load('preprocess/forunet/images/nega_3/'+img_file).astype(np.float64)
...
######################################################################################
        ...
        imgss[j] = im
    np.save(os.path.join("preprocess/forunet/images/nega_processed_3/images_%s.npy" % (filename[i])),imgss)
