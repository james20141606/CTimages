...
def main():
    ...
    f = h5py.File('preprocess/3D/diameter_16_12.9')   #182
    names = f.keys()
    posiname = []
    neganame = []
    for name in names:
    ...
    posi_train = posiname[:127]
    nega_train = neganame[:127]
    ...
    posi_val = posiname[127:]
    nega_val = neganame[127:]
    ...
    shape1 = np.shape(X_train)
    X_train = np.reshape(X_train, [shape1[0], shape1[1],shape1[2],shape1[3],1])
    shape2 = np.shape(X_val)
    X_val = np.reshape(X_val, [shape2[0], shape2[1],shape2[2],shape2[3],1])
    ...
    batch_size = 110      #set to a proper number according to the memory

    x = tf.placeholder(tf.float32, [batch_size, 16, 16, 16, 1])
    ...
    tl.files.save_npz(network.all_params , name='train/model_16.npz')
