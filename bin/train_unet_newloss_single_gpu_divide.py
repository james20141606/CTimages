...
working_path = "preprocess/forunet/segment/"
...
#option: alpha or divide true sigma
def dice_sigma_loss(y_true, y_pred):
    ...
    n = y_true /K.sum(y_true)
    sigma_x_ = K.sum(K.abs(x - K.sum(x*n))*n)
    sigma_y_ = K.sum(K.abs(y - K.sum(y*n))*n)
    loss = - (2. * intersection ) / (K.sum(y_true_f) + K.sum(y_pred_f) ) +(K.sqrt(sigma_x)*K.sqrt(sigma_y)/K.sqrt(sigma_x_)/K.sqrt(sigma_y_))
    return loss
...
def sigma_fraction(y_true, y_pred):
    y, x = np.mgrid[:224, :224]
    y_true_f = K.flatten(y_true)
    y_pred_f = K.flatten(y_pred)
    m = y_pred /K.sum(y_pred)
    ...
    sigma_x_ = K.sum(K.abs(x - K.sum(x*n))*n)
    sigma_y_ = K.sum(K.abs(y - K.sum(y*n))*n)
    sigma =sigma /K.sqrt(sigma_x_)/K.sqrt(sigma_y_)
    return sigma

def sigma_true(y_true,y_pred):
    y, x = np.mgrid[:224, :224]
    m = y_true /K.sum(y_true)
    sigma_x = K.sum(K.abs(x - K.sum(x*m))*m)
    sigma_y = K.sum(K.abs(y - K.sum(y*m))*m)
    sigma = K.sqrt(sigma_x)*K.sqrt(sigma_y)
    return sigma
...
def get_unet():
    ...
    sgd = SGD(lr=0.01, decay=1e-6, momentum=0.9, nesterov=True,clipvalue=0.5)
        ...
        #else:
    model.compile(optimizer=Adam(lr=1.0e-5), loss=dice_sigma_loss, metrics=[dice_coef,sigma,sigma_fraction,sigma_true])
    #model.compile(optimizer=sgd, loss=dice_loss, metrics=[dice_coef,sigma])
    return model
...
def train_and_predict(use_existing):
    ...
    imgs_mask_test = np.ndarray([num_test,1,224,224],dtype=np.float32)
