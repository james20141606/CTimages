...
def main():
    ...
    f = h5py.File('preprocess/3D/diameter_6_12.9')  #82
    names = f.keys()
    posiname = []
    neganame = []
    for name in names:
    ...
    posi_train = posiname[:57]
    nega_train = neganame[:57]
    ...
    posi_val = posiname[57:]
    nega_val = neganame[57:]
    ...
    batch_size = 50     #set to a proper number according to the memory
    x = tf.placeholder(tf.float32, [batch_size, 6, 6, 6, 1])
    ...
    # tl.files.save_npz(network.all_params , name='train/model_6.npz')
    f.close()
