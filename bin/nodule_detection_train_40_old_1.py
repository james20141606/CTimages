...
def main():
    ...
    for epoch in range(n_epoch):
        ...
        if epoch + 1 == 1 or (epoch + 1) % print_freq == 0:
			...
			train_loss, train_acc, n_batch = 0.0, 0.0, 0.0
            for X_train_a, y_train_a in tl.iterate.minibatches(X_train, y_train, batch_size, shuffle=True):
                dp_dict = tl.utils.dict_to_one( network.all_drop )
                feed_dict = {x: X_train_a, y_: y_train_a}
                feed_dict.update(dp_dict)
                # err, ac = sess.run([cost, acc], feed_dict=feed_dict)
                summary, err, acc = sess.run([merged, cost, accuracy], feed_dict=feed_dict)
				step_train += 1
				train_writer.add_summary(summary, step_train)
				train_loss += err; train_acc += acc; n_batch += 1
            print("   train loss: %f" % (train_loss/ n_batch))
            print("   train acc: %f" % (train_acc/ n_batch))

			

			val_loss, val_acc, n_batch = 0.0, 0.0, 0.0
            for X_val_a, y_val_a in tl.iterate.minibatches(
                ...
                summary, err, acc = sess.run([merged, cost, accuracy], feed_dict=feed_dict)
				step_val += 1 
				test_writer.add_summary(val_summary, step_val)
            	val_loss += err; val_acc += acc; n_batch += 1
            print("   train loss: %f" % (val_loss/ n_batch))
            print("   train acc: %f" % (val_acc/ n_batch))			
